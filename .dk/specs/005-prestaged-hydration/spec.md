...
- **FR-009**: System MUST write per-source run state (status, row_count, started_at, finished_at, artifact_sha256) directly to the metadata tracking table, independent of external observability services.
...
- **FR-011**: System MUST be idempotent across restarts: a re-run MUST skip tables already marked complete in the metadata tracking table for the same `run_id`, where `run_id` is a deterministic hash of (sorted artifact sha256s, target cluster fingerprint).
