...
QUALITY_CHECKS = {
    'cms_medicare_inpatient': {
        'total_records': "SELECT COUNT(*) FROM hcs_raw.cms_inpatient_puf",
        'null_provider_id': "SELECT COUNT(*) FROM hcs_raw.cms_inpatient_puf WHERE provider_id IS NULL",
        'null_drg_code': "SELECT COUNT(*) FROM hcs_raw.cms_inpatient_puf WHERE drg_code IS NULL",
        'invalid_discharges': "SELECT COUNT(*) FROM hcs_raw.cms_inpatient_puf WHERE total_discharges < 0",
        'tavr_records': "SELECT COUNT(*) FROM hcs_raw.cms_inpatient_puf WHERE drg_code IN ('266', '267')",
    },
    'cms_hospital_info': {
        'total_records': "SELECT COUNT(*) FROM hcs_raw.cms_hospital_general_info",
        'null_provider_id': "SELECT COUNT(*) FROM hcs_raw.cms_hospital_general_info WHERE provider_id IS NULL",
        'null_state': "SELECT COUNT(*) FROM hcs_raw.cms_hospital_general_info WHERE state IS NULL",
        'invalid_rating': "SELECT COUNT(*) FROM hcs_raw.cms_hospital_general_info WHERE hospital_overall_rating NOT BETWEEN 1 AND 5 AND hospital_overall_rating IS NOT NULL",
    },
    'cms_cost_reports': {
        'total_records': "SELECT COUNT(*) FROM hcs_raw.cms_cost_reports_puf",
        'null_provider_id': "SELECT COUNT(*) FROM hcs_raw.cms_cost_reports_puf WHERE provider_id IS NULL",
        'negative_revenue': "SELECT COUNT(*) FROM hcs_raw.cms_cost_reports_puf WHERE net_patient_revenue < 0",
        'invalid_margin': "SELECT COUNT(*) FROM hcs_raw.cms_cost_reports_puf WHERE operating_margin < -1 OR operating_margin > 1",
    },
    'acc_tvc': {
        'total_records': "SELECT COUNT(*) FROM hcs_raw.acc_tvc",
        'null_facility_name': "SELECT COUNT(*) FROM hcs_raw.acc_tvc WHERE facility_name IS NULL",
        'expired_certs': "SELECT COUNT(*) FROM hcs_raw.acc_tvc WHERE expiration_date < CURRENT_DATE",
        'future_cert_date': "SELECT COUNT(*) FROM hcs_raw.acc_tvc WHERE certification_date > CURRENT_DATE",
    },
    'hrsa_shortage_areas': {
        'total_records': "SELECT COUNT(*) FROM hcs_raw.hrsa",
        'null_hpsa_id': "SELECT COUNT(*) FROM hcs_raw.hrsa WHERE hpsa_id IS NULL",
        'null_state': "SELECT COUNT(*) FROM hcs_raw.hrsa WHERE state_abbr IS NULL",
        'invalid_score': "SELECT COUNT(*) FROM hcs_raw.hrsa WHERE hpsa_score < 0 OR hpsa_score > 25",
    },
}
