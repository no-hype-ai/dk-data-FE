"""CMS Outpatient PUF Fetcher.

Fetches outpatient hospital HCPCS-level utilization data from the CMS
Medicare Outpatient Hospitals dataset. Provides service volumes,
charges, and payment data at the provider/HCPCS level.

Feature: 016-cms-puf-datasource-integration (Phase 3)

Source: https://data.cms.gov/provider-summary-by-type-of-service/medicare-outpatient-hospitals
"""
...
class CMSOutpatientPUFFetcher(BaseFetcher):
    """Fetcher for CMS Outpatient PUF (HCPCS-level volumes and payments)."""
    ...
    def fetch(self, **kwargs) -> Dict[str, Any]:
        """Fetch outpatient PUF records via paginated JSON API.
        ...
        try:
            ...
            logger.exception("Outpatient PUF fetch failed: %s", exc)
            result = {
                "status": "failed",
                "records": [],
                "hash": None,
    ...
    def _fetch_paginated(
        ...
        """Page through the CMS Outpatient API.
        ...
        while True:
            ...
            logger.debug("Fetching Outpatient PUF page offset=%d", offset)

            self._last_offset = offset
            try:
                data = self.fetch_json(self.API_ENDPOINT, params=params)
            except Exception as exc:
                logger.warning("Outpatient PUF page error at offset %d: %s", offset, exc)
                break
        ...
        logger.info("Fetched %d Outpatient PUF records", len(records))
        return records
    ...
    def _save_and_hash(
        ...
        filename = f"cms_outpatient_puf{year_suffix}_{timestamp}.json"
