"""CMS Hospital Outpatient PUF fetcher."""
import logging
...
class CMSOutpatientPUFFetcher(BaseFetcher):
    ...
    DATASET_UUID = "ccbc9a44-40d4-46b4-a709-5caa59212e50"
        ...
        try:
            ...
            if years:
                total_fetched, total_inserted = self._stream_cms_api_multi_year_to_db(
                    self.DATASET_UUID, load_cms_outpatient_puf, years=years, max_records_per_year=max_records
                )
            else:
                total_fetched, total_inserted = self._stream_cms_api_to_db(
                    self.DATASET_UUID, load_cms_outpatient_puf, source_year=source_year, max_records=max_records
                )
