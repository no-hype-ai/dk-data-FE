"""CMS Hospital Readmissions Reduction Program (HRRP) Fetcher.

Fetches annual HRRP data from the CMS Provider Data API.
HRRP publishes excess-readmission ratios and payment-reduction factors
per hospital across six condition cohorts (AMI, HF, pneumonia, COPD,
CABG, THA/TKA).
...
class CMSHRRPFetcher(BaseFetcher):
    """Fetcher for CMS Hospital Readmissions Reduction Program data."""

    SOURCE_NAME = "cms_hrrp"
    BASE_URL = "https://data.cms.gov/provider-data/dataset/9n3s-kdb3"
    ...
    def get_latest_url(self) -> str:
        """Return the API endpoint for HRRP data."""
        return f"{self.API_ENDPOINT}?limit={DEFAULT_PAGE_SIZE}&offset=0"

    def fetch(self, **kwargs) -> Dict[str, Any]:
        """Fetch HRRP records via JSON API.
        ...
        try:
            ...
            logger.exception("HRRP fetch failed: %s", exc)
            result = {
                "status": "failed",
                "records": [],
                "hash": None,
    ...
    def _fetch_paginated(
        ...
        while True:
            ...
            logger.debug("Fetching HRRP page offset=%d", offset)
            ...
            try:
                ...
                logger.warning(
                    "HRRP page error at offset %d: %s", offset, exc
                )
                break
        ...
        logger.info("Fetched %d HRRP records", len(records))
        return records
    ...
    def _save_and_hash(self, records: List[Dict]) -> Optional[str]:
        ...
        filename = f"cms_hrrp_{timestamp}.json"
