"""CMS Hospital-Acquired Condition (HAC) Reduction Program Fetcher.

Fetches annual HAC Reduction Program data from the CMS Provider Data API.
The HAC program penalises hospitals in the worst-performing quartile on
hospital-acquired condition measures (CLABSI, CAUTI, SSI, MRSA, CDI, PSI 90).

Source: https://data.cms.gov/provider-data/dataset/yq43-i98g
"""
...
class CMSHACReductionFetcher(BaseFetcher):
    """Fetcher for CMS HAC Reduction Program data."""

    SOURCE_NAME = "cms_hac_reduction"
    BASE_URL = "https://data.cms.gov/provider-data/dataset/yq43-i98g"
    ...
    def get_latest_url(self) -> str:
        """Return the API endpoint for HAC Reduction Program data."""
        return f"{self.API_ENDPOINT}?limit={DEFAULT_PAGE_SIZE}&offset=0"

    def fetch(self, **kwargs) -> Dict[str, Any]:
        """Fetch HAC Reduction Program records via JSON API.
        ...
        try:
            ...
            logger.exception("HAC Reduction fetch failed: %s", exc)
            result = {
                "status": "failed",
                "records": [],
                "hash": None,
    ...
    def _fetch_paginated(
        ...
        """Page through the HAC Reduction Program API."""
        ...
        while True:
            ...
            logger.debug("Fetching HAC Reduction page offset=%d", offset)
            ...
            try:
                ...
                logger.warning(
                    "HAC Reduction page error at offset %d: %s", offset, exc
                )
                break
        ...
        logger.info("Fetched %d HAC Reduction records", len(records))
        return records
    ...
    def _save_and_hash(self, records: List[Dict]) -> Optional[str]:
        ...
        filename = f"cms_hac_reduction_{timestamp}.json"
