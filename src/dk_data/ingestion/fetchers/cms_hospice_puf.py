"""CMS Hospice Provider PUF fetcher."""
import logging
...
class CMSHospicePUFFetcher(BaseFetcher):
    SOURCE_NAME = "cms_hospice_puf"
    # Canonical UUID — fetches most recent available year.
    # Pass years=[2021, 2022, 2023] to backfill multiple years dynamically.
    DATASET_UUID = "4e73f1b5-82cb-4682-8ad2-28493f0b6840"
        ...
        try:
            ...
            if years:
                total_fetched, total_inserted = self._stream_cms_api_multi_year_to_db(
                    self.DATASET_UUID, load_cms_hospice_puf, years=years, max_records_per_year=max_records
                )
            else:
                total_fetched, total_inserted = self._stream_cms_api_to_db(
                    self.DATASET_UUID, load_cms_hospice_puf, source_year=source_year, max_records=max_records
                )
