...
def load_cms_physician_puf_services(filepath: Optional[str] = None, rows: Optional[List[Dict]] = None, source_year: int = 2023, max_records: int = 0, source_hash: Optional[str] = None) -> dict:
    """Load CMS Physician PUF by Provider and Service (HCPCS grain)."""
    logger.info(f"Loading CMS Physician PUF Services (year={source_year})")
