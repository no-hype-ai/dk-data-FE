"""Source loader for CMS HAC Reduction Program data."""
import hashlib
import json
import logging
...
TABLE = 'hcs_raw.cms_hac_reduction'
API_ENDPOINT = 'cms_hac_reduction'


def load_cms_hac_reduction_data(
    records: List[Dict[str, Any]],
    source_hash: Optional[str] = None,
    source_file: Optional[str] = None,
    batch_size: int = 500,
) -> Dict[str, Any]:
    """Load CMS HAC Reduction Program records into hcs_raw.cms_hac_reduction as JSONB."""
    if not records:
        ...
        with conn.cursor() as cur:
            for idx, raw_record in enumerate(records):
                try:
                    ...
                    request_id = str(raw_record.get('facility_id', f'{SOURCE_NAME}_{idx}'))

                    cur.execute("""
                        INSERT INTO hcs_raw.cms_hac_reduction (
