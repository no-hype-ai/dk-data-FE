...
async def register_data_source(request: DataSourceRegistration):
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            await conn.execute("""
                INSERT INTO meta.sync_schedules
...
async def delete_data_source(source_name: str, delete_data: bool = Query(False)):
    ...
    try:
        ...
        async with pool.acquire() as conn:
            # Check if source exists
            row = await conn.fetchrow(
                "SELECT source, options FROM meta.sync_schedules WHERE source = $1",
                source_name
            )
            if not row:
                raise HTTPException(status_code=404, detail=f"Data source '{source_name}' not found")
            ...
            await conn.execute(
                "DELETE FROM meta.sync_schedules WHERE source = $1",
                source_name
            )
...
async def list_credentials(source_name: str):
    ...
    try:
        ...
        async with pool.acquire() as conn:
            row = await conn.fetchrow("""
                SELECT options FROM meta.sync_schedules WHERE source = $1
            """, source_name)
        ...
        async with pool.acquire() as conn:
            ...
            if clear_existing:
                target_table = await conn.fetchval("""
                    SELECT options->>'target_table' FROM meta.sync_schedules WHERE source = $1
                """, source_name)
                if target_table:
                    result = await conn.execute(f"DELETE FROM {target_table}")
