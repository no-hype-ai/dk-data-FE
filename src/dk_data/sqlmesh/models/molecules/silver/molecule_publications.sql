...
-- FULL kind (weekly rebuild) avoids MERGE, so UNION ALL cross-branch duplicates
-- are not an issue — SQLMesh replaces the table from scratch each run.
...
ORDER BY molecule_id, publication_id, confidence DESC
