-- SQLMesh Model: Bronze FDA Untitled Letters
-- Passthrough from mol_raw.fda_untitled_letters to typed bronze layer.
-- Feature: 006-claims-engine-data-gaps (T074)

MODEL (
    name mol_bronze.fda_untitled_letters,
...
SELECT
    ...
    r.company_address::TEXT             AS company_address,
    r.subject::TEXT                     AS subject,
    r.issuing_office::TEXT              AS issuing_office,
    r.response_letter_url::TEXT         AS response_letter_url,
    r.closeout_letter_url::TEXT         AS closeout_letter_url,
    r.full_text::TEXT                   AS full_text,
    r.drug_mentions::JSONB              AS drug_mentions,

    -- Source tracking
    'fda_untitled_letters'              AS source,
    r.ingested_at                       AS source_updated_at,
    FALSE                               AS processed_to_silver,
    r.ingested_at                       AS ingested_at

FROM mol_raw.fda_untitled_letters r
