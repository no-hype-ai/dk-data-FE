...
WITH prescriber_agg AS (
    ...
    WHERE year = (SELECT MAX(year) FROM hcs_bronze.cms_part_d_prescriber)
    GROUP BY npi
),
...
procedure_agg AS (
    ...
    WHERE year = (SELECT MAX(year) FROM hcs_bronze.cms_physician_puf)
    GROUP BY npi
),
...
LEFT JOIN hcs_bronze.cms_nucc nucc ON n.taxonomy_code = nucc.taxonomy_code
