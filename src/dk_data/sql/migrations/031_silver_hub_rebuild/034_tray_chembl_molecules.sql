-- Migration 031/034_tray_chembl_molecules.sql: Tray procedure for mol_bronze.chembl_molecules
-- Feature: 001-silver-medallion-rebuild / T151-T154
--
-- Atomically rebuilds mol_bronze.chembl_molecules via unlogged tray table.
...
DECLARE
    ...
    v_tray_name     CONSTANT TEXT    := 'chembl_molecules_tray';
    v_target_name   CONSTANT TEXT    := 'chembl_molecules';
    v_old_name      CONSTANT TEXT    := 'chembl_molecules_old';
...
BEGIN
    ...
    LOOP
        ...
        EXECUTE format(
            'INSERT INTO %I.%I SELECT * FROM %I.%I ORDER BY molregno LIMIT %L OFFSET %L',
        ...
        INSERT INTO meta.transform_runs (
            ...
            'mol_bronze.refresh_chembl_molecules_via_tray', v_chunk_pos, v_chunk_rows, v_wal_bytes, NOW()
    ...
    RAISE NOTICE 'refresh_chembl_molecules_via_tray complete: % rows inserted', v_rows_inserted;
END;
$$;
