-- Migration 031/033_tray_pubchem.sql: Tray procedure for mol_bronze.pubchem
-- Feature: 001-silver-medallion-rebuild / T151-T154
--
-- Atomically rebuilds mol_bronze.pubchem via unlogged tray table.
...
DECLARE
    ...
    v_tray_name     CONSTANT TEXT    := 'pubchem_tray';
    v_target_name   CONSTANT TEXT    := 'pubchem';
    v_old_name      CONSTANT TEXT    := 'pubchem_old';
...
BEGIN
    ...
    LOOP
        ...
        EXECUTE format(
            'INSERT INTO %I.%I SELECT * FROM %I.%I ORDER BY cid LIMIT %L OFFSET %L',
        ...
        INSERT INTO meta.transform_runs (
            ...
            'mol_bronze.refresh_pubchem_via_tray', v_chunk_pos, v_chunk_rows, v_wal_bytes, NOW()
    ...
    RAISE NOTICE 'refresh_pubchem_via_tray complete: % rows inserted', v_rows_inserted;
END;
$$;
