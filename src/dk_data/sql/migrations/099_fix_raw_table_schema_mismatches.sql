...
-- =============================================================================
-- 1. hcs_raw.cms_care_compare — table doesn't exist (083 created cms_care_compare_physicians)
...
CREATE INDEX IF NOT EXISTS idx_cms_care_compare_state ON hcs_raw.cms_care_compare (state);

-- =============================================================================
-- 2. hcs_raw.cms_hospital_general_info — loader uses facility_id, facility_name, etc.
...
-- =============================================================================
-- 3. hcs_raw.cms_hospital_quality — loader uses facility_id + star ratings
...
-- =============================================================================
ALTER TABLE hcs_raw.cms_hospital_quality
    ADD COLUMN IF NOT EXISTS facility_id TEXT,
    ADD COLUMN IF NOT EXISTS facility_name TEXT,
...
-- =============================================================================
-- 4. hcs_raw.cms_ndc — loader uses product_ndc, brand_name, generic_name, marketing_category
--    Migration has ndc, proprietary_name, nonproprietary_name
-- =============================================================================
ALTER TABLE hcs_raw.cms_ndc
...
-- =============================================================================
-- 5. hcs_raw.cms_nppes — loader uses entity_type_code, provider_* column names
...
-- =============================================================================
-- 6. hcs_raw.cms_nucc — loader uses taxonomy_type, migration has provider_type
-- =============================================================================
ALTER TABLE hcs_raw.cms_nucc
    ADD COLUMN IF NOT EXISTS taxonomy_type TEXT;

-- =============================================================================
-- 7. hcs_raw.cms_part_b_spending — loader uses brand_name, generic_name
...
-- =============================================================================
-- 8. hcs_raw.cms_pos — loader uses street_address, provider_type, beds, ownership_type
--    Migration has address, facility_type, bed_count
-- =============================================================================
ALTER TABLE hcs_raw.cms_pos
