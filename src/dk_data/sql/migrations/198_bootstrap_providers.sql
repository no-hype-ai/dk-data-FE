-- T079: Bootstrap hcs_silver.providers from hcs_bronze.cms_nppes
-- Expected: ~10M rows (LARGEST — run last)
-- Chunk: ≤50K rows, ≤200 MB WAL per chunk, pg_sleep(0.05) between chunks

CREATE OR REPLACE PROCEDURE hcs_silver.bootstrap_providers()
LANGUAGE plpgsql
AS $$
DECLARE
    v_proc_name  text := 'hcs_silver.bootstrap_providers';
    v_chunk_size int  := 50000;
    v_resume_pos bigint := 0;
    v_new_pos    bigint;
    v_rows       bigint;
    v_start_lsn  pg_lsn;
    v_end_lsn    pg_lsn;
    v_started_at timestamptz;
BEGIN
    -- 1. Acquire job lock (delete stale first, then insert)
    DELETE FROM meta.job_locks WHERE name = v_proc_name AND expires_at < NOW();
    INSERT INTO meta.job_locks (name, locked_by, locked_at, expires_at)
    VALUES (v_proc_name, pg_backend_pid()::text, NOW(), NOW() + INTERVAL '8 hours')
    ON CONFLICT (name) DO NOTHING;

    IF NOT FOUND THEN
        RAISE EXCEPTION 'Could not acquire job lock for %; another instance may be running', v_proc_name;
    END IF;

    -- 2. Initialize or resume meta.refresh_state
    INSERT INTO meta.refresh_state (procedure_name, last_chunk_position, last_commit_at, status)
    VALUES (v_proc_name, '0', NOW(), 'in_progress')
    ON CONFLICT (procedure_name) DO UPDATE
        SET status = 'in_progress', last_commit_at = NOW()
        WHERE meta.refresh_state.status <> 'in_progress';

    SELECT last_chunk_position::bigint INTO v_resume_pos
    FROM meta.refresh_state
    WHERE procedure_name = v_proc_name;

    v_resume_pos := COALESCE(v_resume_pos, 0);

    -- 3. Chunked load loop
    LOOP
        v_started_at := clock_timestamp();
        v_start_lsn  := pg_current_wal_lsn();

        WITH inserted AS (
            INSERT INTO hcs_silver.providers (
                npi,
                pecos_id,
                last_name,
                first_name,
                middle_name,
                credential,
                primary_taxonomy_code,
                primary_specialty,
                state
            )
            SELECT
                src.npi,
                src.pecos_id,
                src.provider_last_name_legal_name      AS last_name,
                src.provider_first_name                AS first_name,
                src.provider_middle_name               AS middle_name,
                src.provider_credential_text           AS credential,
                src.healthcare_provider_taxonomy_code_1 AS primary_taxonomy_code,
                src.healthcare_provider_taxonomy_description_1 AS primary_specialty,
                src.provider_business_practice_location_address_state_name AS state
            FROM hcs_bronze.cms_nppes src
            WHERE src.entity_type_code = '1'           -- Type 1 = individual providers
              AND src.npi IS NOT NULL
              AND src.id > v_resume_pos
            ORDER BY src.id
            LIMIT v_chunk_size
            ON CONFLICT (npi) DO NOTHING
            RETURNING provider_id, npi
        ),
        id_insert AS (
            INSERT INTO hcs_silver.provider_identifiers (source, identifier, provider_id, is_primary)
            SELECT
                'npi',
                i.npi,
                i.provider_id,
                true
            FROM inserted i
            ON CONFLICT (source, identifier) DO NOTHING
        ),
        pecos_id_insert AS (
            INSERT INTO hcs_silver.provider_identifiers (source, identifier, provider_id, is_primary)
            SELECT
                'pecos',
                src.pecos_id,
                i.provider_id,
                false
            FROM inserted i
            JOIN hcs_bronze.cms_nppes src ON src.npi = i.npi
            WHERE src.pecos_id IS NOT NULL
            ON CONFLICT (source, identifier) DO NOTHING
        ),
        name_insert AS (
            INSERT INTO hcs_silver.provider_names (normalized_name, provider_id, name_kind, source, display_name)
            SELECT
                -- canonical_name format: "last_name, first_name"
                LOWER(TRIM(src.provider_last_name_legal_name || ', ' || src.provider_first_name)),
                i.provider_id,
                'canonical',
                'cms_nppes',
                src.provider_last_name_legal_name || ', ' || src.provider_first_name
            FROM inserted i
            JOIN hcs_bronze.cms_nppes src ON src.npi = i.npi
            WHERE src.provider_last_name_legal_name IS NOT NULL
              AND src.provider_first_name IS NOT NULL
            ON CONFLICT (normalized_name, provider_id, source) DO NOTHING
        )
        SELECT COUNT(*) INTO v_rows FROM inserted;

        v_end_lsn := pg_current_wal_lsn();

        SELECT COALESCE(MAX(src.id), v_resume_pos) INTO v_new_pos
        FROM hcs_bronze.cms_nppes src
        WHERE src.entity_type_code = '1'
          AND src.npi IS NOT NULL
          AND src.id > v_resume_pos
        ORDER BY src.id
        LIMIT v_chunk_size;

        INSERT INTO meta.transform_runs (procedure_name, chunk_position, started_at, ended_at, rows_processed, wal_bytes)
        VALUES (
            v_proc_name,
            v_new_pos::text,
            v_started_at,
            clock_timestamp(),
            v_rows,
            pg_wal_lsn_diff(v_end_lsn, v_start_lsn)
        );

        UPDATE meta.refresh_state
        SET last_chunk_position = v_new_pos::text,
            last_commit_at      = NOW()
        WHERE procedure_name = v_proc_name;

        COMMIT;

        EXIT WHEN v_rows = 0 OR v_new_pos = v_resume_pos;
        v_resume_pos := v_new_pos;

        PERFORM pg_sleep(0.05);
    END LOOP;

    -- 4. Mark completed
    UPDATE meta.refresh_state
    SET status = 'completed', last_commit_at = NOW()
    WHERE procedure_name = v_proc_name;

    -- 5. Release lock
    DELETE FROM meta.job_locks WHERE name = v_proc_name;

    COMMIT;
END;
$$;
