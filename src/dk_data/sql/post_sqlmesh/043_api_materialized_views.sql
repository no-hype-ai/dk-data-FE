...
--
-- Source tables are created by SQLMesh. On first deploy they may not exist.
-- Each view is wrapped in an exception handler that skips gracefully.
...
BEGIN
    ...
    BEGIN
        EXECUTE $mat$
            ...
            SELECT f.facility_id, f.facility_name, f.facility_type,
                   f.address_city, f.address_state, f.address_country,
                   f.npi, f.cms_certification_number, f.bed_count,
                   f.star_rating, f.last_updated_at
            FROM hcs_gold.dim_hospital f
            WHERE f.confidence_score >= 0.95 AND f.is_active = TRUE
            WITH DATA
        $mat$;
    ...
    BEGIN
        EXECUTE $mat$
            ...
            SELECT m.molecule_id, m.canonical_name, m.canonical_smiles, m.inchi_key,
                   m.molecular_weight, m.drug_type, m.approval_status, m.approval_date,
                   m.max_phase, m.indication_count, m.confidence_score
    ...
    BEGIN
        EXECUTE $mat$
            ...
            SELECT t.trial_id, t.nct_id, t.title, t.phase, t.status,
                   t.start_date, t.completion_date, t.primary_outcome,
                   t.sponsor_name, t.condition_count, t.intervention_count,
                   t.confidence_score
