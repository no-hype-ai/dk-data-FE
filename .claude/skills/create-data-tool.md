# Create New Pipeline Data Retrieval Tool

## Trigger
When the user asks to add a new data retrieval tool, external API source, or data pipeline adapter.
...
Create `src/dk_data/services/pipeline/adapters/{source_name}.py`:

```python
"""Pipeline Adapter: {Source Name}
...
Add the tool to `TOOL_REGISTRY` in `src/dk_data/services/pipeline/tool_registry.py`:
...
"{source-name}-search": ToolDefinition(
    ...
    adapter_module="dk_data.services.pipeline.adapters.{source_name}",
...
Also add the tool to `src/dk_data/services/data_tools/tool_registry.py` with the enhanced `DataToolDefinition` including `supported_query_keys` and `category`.
...
class Test{SourceName}Adapter:
    def test_normalize_extracts_key_fields(self):
        from dk_data.services.pipeline.adapters.{source_name} import Adapter
        adapter = Adapter()
        mock_response = {
            # Sample API response
        }
...
- [ ] Adapter file created (`src/dk_data/services/pipeline/adapters/{source_name}.py`)
- [ ] Adapter implements `normalize()`, `source_name`, `raw_table`, `raw_schema`
- [ ] Tool registered in both `pipeline/tool_registry.py` and `data_tools/tool_registry.py`
...
- [ ] Rate limit entry added to `config/rate_limits.yaml`
...
| `fetch_filter` | Fetch bulk data, filter client-side | 5 |
| `supplementary` | Context/infrastructure data, less frequently updated | 2 |
