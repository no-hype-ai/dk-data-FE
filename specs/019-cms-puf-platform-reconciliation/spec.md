...
- **FR-025**: Every agent MUST route all LLM calls through the shared LiteLLM proxy — no direct API calls to any LLM provider. Agents MUST use the `openai` Python SDK (or `httpx`) pointed at `LITELLM_PROXY_URL` (Doppler-sourced). `import anthropic` is FORBIDDEN in any new agent or loader file introduced by this feature. Agents MUST implement exponential backoff on `429` responses (use `tenacity`) — the LiteLLM proxy has a 500 RPM shared rate limit across all DataKinetic services; agents MUST NOT exceed 5 concurrent LLM calls per agent run. Agents MUST apply a quality escalation pattern: call `pharma-llm` first; if confidence < 0.6, re-call with `claude-sonnet-4-20250514` (one escalation maximum per record)
- **FR-025a**: Each agent run MUST cap its LLM processing at a configurable maximum records per run (default: 50, matching ARCHITECTURE-BEST-PRACTICES.md MAX_EVIDENCE_PER_PILLAR). Agents accept a `--limit` CLI argument. Unbounded agent runs that process millions of rows in a single invocation are forbidden — subsequent records are deferred to the next scheduled run. This prevents cost explosion and protects the shared 500 RPM LiteLLM rate limit.
