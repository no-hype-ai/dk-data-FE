...
**Scale/Scope**: 28 CMS bulk file sources (annual CSV, up to ~2GB each), 6 API sources (incremental daily/weekly, 2 new + 4 existing extended), 7 agents (monthly + on-demand), 21 SQLMesh models; SOURCES dict grows from **22 current entries** to **52 post-feature** (22 + 30 new); validate-staging-ingestion.sh count check changes from `= "22"` to `-ge 52`
