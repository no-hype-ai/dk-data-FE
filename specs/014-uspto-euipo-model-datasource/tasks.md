...
- [ ] T018 [US3] Update `src/dk_data/sqlmesh/models/molecules/gold/molecule_profile.sql` — add new CTE `trademark_info` that JOINs `silver.trademarks` to `silver.molecule_aliases` on `LOWER(mark_name) = LOWER(alias_name)` where `alias_type IN ('brand', 'trade', 'product', 'canonical')`. This links trademarks via known brand/trade names rather than generic names (trademarks are filed as brand names like "Humira", not INN names like "adalimumab"). Aggregate: trademark_count (COUNT DISTINCT on trademark_identifier||source), active_trademark_count (status IN ('Registered', 'REGISTERED')), us_trademark_count (source='uspto_trademarks'), eu_trademark_count (source='euipo_trademarks'), latest_us_trademark_status, latest_eu_trademark_status. LEFT JOIN trademark_info in final SELECT. Add 6 new columns with COALESCE defaults. See `contracts/gold-models.md`.
...
- [ ] T019 [US5] Update `src/dk_data/observability/metrics.py` — **NOTE**: The existing metrics.py has Prometheus objects (`DATA_SOURCE_ROW_COUNT`, `DATA_SOURCE_LAST_REFRESH_TIMESTAMP`, `DATA_SOURCE_STALENESS_HOURS`) and helper function `record_data_source_refresh(source_id, source_name, row_count)`. It does NOT have `local_sources`, `raw_sources`, `layer_tables`, or `external_sources` dicts. To integrate IP sources: (1) Add a new `IP_DATA_SOURCES` list constant: `['uspto_patents', 'uspto_ci', 'epo_patents', 'uspto_trademarks', 'euipo_trademarks']` for reference. (2) Optionally add new Gauges for IP-specific tracking: `dk_source_health_status` (source label), `dk_table_record_count` (layer + table_name labels). (3) Add helper function `record_ip_source_health(source_name, is_healthy)` that sets the health gauge. (4) The existing `record_data_source_refresh()` function already supports arbitrary source_id/source_name labels — callers just need to pass IP source names. Read `metrics.py` fully before implementing to match existing Prometheus patterns.
...
- [ ] T024 [US6] Update `k8s/base/kustomization.yaml` — add two new resources under the "# Molecule data sources" comment block: `- ingestion/cronjob-fetch-uspto-trademarks.yaml` and `- ingestion/cronjob-fetch-euipo.yaml`. Add comment: `# Trademark data sources (014-uspto-euipo-model-datasource)`.
