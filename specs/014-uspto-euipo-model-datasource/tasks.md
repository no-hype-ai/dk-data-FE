...
- [ ] T019 [US5] Update `src/dk_data/observability/metrics.py` — **NOTE**: The existing metrics.py has Prometheus objects (`DATA_SOURCE_ROW_COUNT`, `DATA_SOURCE_LAST_REFRESH_TIMESTAMP`, `DATA_SOURCE_STALENESS_HOURS`) and helper function `record_data_source_refresh(source_id, source_name, row_count)`. It does NOT have `local_sources`, `raw_sources`, `layer_tables`, or `external_sources` dicts. To integrate IP sources: (1) Add a new `IP_DATA_SOURCES` list constant: `['uspto_patents', 'uspto_ci', 'epo_patents', 'uspto_trademarks', 'euipo_trademarks']` for reference. (2) Add new Gauges (REQUIRED by spec.md US5 acceptance criteria): `dk_source_health_status` Gauge with `source` label, and `dk_table_record_count` Gauge with `layer` + `table_name` labels — follow existing Gauge naming convention with `dk_` prefix. (3) Add helper function `record_ip_source_health(source_name, is_healthy)` that sets the health gauge. (4) The existing `record_data_source_refresh()` function already supports arbitrary source_id/source_name labels — callers (loaders) should call `record_data_source_refresh(source_name, source_name, records_inserted)` after a successful load batch. Read `metrics.py` fully before implementing to match existing Prometheus patterns (all metrics use `prometheus_client` Gauge/Counter/Histogram, no dict-based config).
