"""Tests for the openalex adapter (#415 WS3 Batch C)."""

import asyncio
import pytest

from dk_data.services.mcp.adapters.openalex import Adapter
...
class TestContract:
    def test_contract(self):
        ...
        a = Adapter()
        assert a.source_name == "openalex"
        assert a.raw_table == "openalex"
        assert a.raw_schema == "mol_raw"
        assert a.normalize({"x": 1}) == {"x": 1}
        assert a.full_table_name == "mol_raw.openalex"
...
class TestDbHit:
    def test_db_hit_returns_dict(self):
        ...
        assert result == {"source": "openalex_local", "results": [{"response_body": {"hit": True}}]}
