"""Tests for the cms_hospital_info adapter (#415 WS3 Batch E — hcs_raw)."""

import asyncio
import pytest

from dk_data.services.mcp.adapters.cms_hospital_info import Adapter
...
class TestContract:
    def test_contract(self):
        ...
        a = Adapter()
        assert a.source_name == "cms_hospital_info"
        assert a.raw_table == "cms_hospital_info"
        assert a.raw_schema == "hcs_raw"
        assert a.normalize({"x": 1}) == {"x": 1}
        assert a.full_table_name == "hcs_raw.cms_hospital_info"
...
class TestDbHit:
    def test_db_hit_returns_dict(self):
        ...
        assert result == {"source": "cms_hospital_info_local", "results": [{"response_body": {"hit": True}}]}
