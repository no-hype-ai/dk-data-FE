"""Tests for cms_chronic_conditions loader (019-cms-puf-platform-reconciliation T017)."""
...
CSV_CONTENT = (
    ...
    "Bene_FIPS_Cd,Bene_Age_Lvl,Bene_Sex_Cd,Bene_Race_Cd,"
    "Bene_Dual_Stus_Cd,Chronic_Condition,Prevalence\n"
    "State,TX,Texas,,48000,65+,1,1,0,Diabetes,27.5\n"
)

MODULE = "dk_data.ingestion.sources.cms_chronic_conditions"


def test_loader_returns_success_result_shape():
    from dk_data.ingestion.sources.cms_chronic_conditions import (
        load_cms_chronic_conditions,
    )
    ...
    with patch(f"{MODULE}.get_cursor") as mock_gc, patch(
        ...
        result = load_cms_chronic_conditions(path)
...
def test_loader_does_not_call_log_to_meta():
    ...
    with patch(f"{MODULE}.get_cursor") as mock_gc, patch(
        ...
        mock_gc.return_value = _get_cursor_ctx(0)
        mock_ur.return_value = 1
        load_cms_chronic_conditions(path)
        mock_ltm.assert_not_called()


def test_fetcher_source_name():
    from dk_data.ingestion.fetchers.cms_chronic_conditions import (
        CMSChronicConditionsFetcher,
    )

    assert CMSChronicConditionsFetcher.SOURCE_NAME == "cms_chronic_conditions"
