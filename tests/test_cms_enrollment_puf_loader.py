"""Tests for cms_enrollment_puf loader (019-cms-puf-platform-reconciliation T017)."""
...
CSV_CONTENT = (
    "State,State Name,County Name,FIPS Code,"
    "Tot_Benes,Orgnl_Mdcr_Benes,MA_and_Oth_Benes\n"
    "CA,California,Los Angeles,06037,1200000,500000,700000\n"
)

MODULE = "dk_data.ingestion.sources.cms_enrollment_puf"


def test_loader_returns_success_result_shape():
    from dk_data.ingestion.sources.cms_enrollment_puf import load_cms_enrollment_puf
    ...
    with patch(f"{MODULE}.get_cursor") as mock_gc, patch(
        ...
        result = load_cms_enrollment_puf(path)
...
def test_loader_does_not_call_log_to_meta():
    ...
    with patch(f"{MODULE}.get_cursor") as mock_gc, patch(
        ...
        mock_gc.return_value = _get_cursor_ctx(0)
        mock_ur.return_value = 1
        load_cms_enrollment_puf(path)
        mock_ltm.assert_not_called()


def test_fetcher_source_name():
    from dk_data.ingestion.fetchers.cms_enrollment_puf import CMSEnrollmentPUFFetcher

    assert CMSEnrollmentPUFFetcher.SOURCE_NAME == "cms_enrollment_puf"
