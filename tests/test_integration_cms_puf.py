...
def test_fetcher_instantiation():
    ...
    report(f"CMS sources in registry", len(cms_sources) >= 28, f"{len(cms_sources)} found")
    ...
    report(f"  API-fetched sources", len(api_sources) >= 25, f"{len(api_sources)} with fetcher")
    report(f"  Legacy file-based loaders", True, f"{len(legacy_sources)}: {sorted(legacy_sources.keys())}")
    ...
    for name, kwargs in test_sources.items():
        ...
        try:
            ...
            if isinstance(records, list) and records:
                sample = records[0]
                report(f"    record keys", True, f"{list(sample.keys())[:6]}...")
        except Exception as e:
...
async def test_aggregation_service():
    ...
    try:
        from dk_data.services.data_platform.gold_aggregation import GoldAggregationService
    ...
    try:
        from dk_data.services.pipeline.silver_gold_refresher import SilverGoldRefresher
...
async def main():
    print(f"\n{'='*65}")
    print(f" CMS PUF Datasource Integration Tests (Spec 016)")
