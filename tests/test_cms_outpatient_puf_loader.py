"""Tests for cms_outpatient_puf loader (019-cms-puf-platform-reconciliation T017)."""
...
CSV_CONTENT = (
    ...
    "Provider State,Provider Zip Code,APC,Outpatient Services,"
    "Average Estimated Submitted Charges,Average Total Payments\n"
    "100001,Outpatient Center,400 Maple Ave,Atlanta,GA,30301,"
    "0012,250,3500.00,1200.00\n"
)

MODULE = "dk_data.ingestion.sources.cms_outpatient_puf"


def test_loader_returns_success_result_shape():
    from dk_data.ingestion.sources.cms_outpatient_puf import load_cms_outpatient_puf
    ...
    with patch(f"{MODULE}.get_cursor") as mock_gc, patch(
        ...
        result = load_cms_outpatient_puf(path)
...
def test_loader_does_not_call_log_to_meta():
    ...
    with patch(f"{MODULE}.get_cursor") as mock_gc, patch(
        ...
        mock_gc.return_value = _get_cursor_ctx(0)
        mock_ur.return_value = 1
        load_cms_outpatient_puf(path)
        mock_ltm.assert_not_called()


def test_fetcher_source_name():
    from dk_data.ingestion.fetchers.cms_outpatient_puf import CMSOutpatientPUFFetcher

    assert CMSOutpatientPUFFetcher.SOURCE_NAME == "cms_outpatient_puf"
