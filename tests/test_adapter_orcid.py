"""Tests for the orcid adapter (#415 WS3 Batch B)."""

import asyncio
import pytest

from dk_data.services.mcp.adapters.orcid import Adapter, OrcidTool
...
class TestContract:
    def test_contract(self):
        ...
        a = Adapter()
        assert a.source_name == "orcid"
        assert a.raw_table == "orcid"
        assert a.raw_schema == "mol_raw"
        assert a.normalize({"x": 1}) == {"x": 1}
        assert a.full_table_name == "mol_raw.orcid"
...
class TestDbHit:
    def test_db_hit_returns_dict(self):
        ...
        assert result == {"source": "orcid_local", "results": [{"response_body": {"hit": True}}]}
...
class TestExistingToolIntact:
    def test_orcid_tool_importable(self):
        assert OrcidTool is not None
