"""Tests for the clinicaltrials adapter (#415 WS3 Batch A). asyncio.run, fake pool, no respx."""
...
class TestContract:
    def test_contract(self):
        ...
        a = Adapter()
        assert a.source_name == "clinicaltrials"
        assert a.raw_table == "clinicaltrials"
        assert a.raw_schema == "mol_raw"
        assert a.normalize({"x": 1}) == {"x": 1}
        assert a.full_table_name == "mol_raw.clinicaltrials"
...
class TestDbHit:
    def test_db_hit_returns_dict(self):
        ...
        assert result == {"source": "clinicaltrials_local", "results": [{"response_body": {"hit": True}}]}
