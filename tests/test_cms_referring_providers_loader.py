"""Tests for cms_referring_providers loader (019-cms-puf-platform-reconciliation T017)."""
...
CSV_CONTENT = (
    "Rndrng_NPI,Rfr_NPI,Rfr_Prvdr_Last_Org_Name,Rfr_Prvdr_First_Name,"
    "Rfr_Prvdr_Type,Rfr_Prvdr_State_Abrvtn,Tot_Rndrng_Prvdrs,"
    "Tot_Srvcs,Tot_Mdcr_Alowd_Amt,Tot_Mdcr_Pymt_Amt\n"
    "1111111111,2222222222,Doe,Jane,Family Medicine,NY,5,120,6000.00,4800.00\n"
)

MODULE = "dk_data.ingestion.sources.cms_referring_providers"


def test_loader_returns_success_result_shape():
    from dk_data.ingestion.sources.cms_referring_providers import (
        load_cms_referring_providers,
    )
    ...
    with patch(f"{MODULE}.get_cursor") as mock_gc, patch(
        ...
        result = load_cms_referring_providers(path)
...
def test_loader_does_not_call_log_to_meta():
    ...
    with patch(f"{MODULE}.get_cursor") as mock_gc, patch(
        ...
        mock_gc.return_value = _get_cursor_ctx(0)
        mock_ur.return_value = 1
        load_cms_referring_providers(path)
        mock_ltm.assert_not_called()


def test_fetcher_source_name():
    from dk_data.ingestion.fetchers.cms_referring_providers import (
        CMSReferringProvidersFetcher,
    )

    assert CMSReferringProvidersFetcher.SOURCE_NAME == "cms_referring_providers"
