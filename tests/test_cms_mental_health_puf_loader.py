"""Tests for cms_mental_health_puf loader (019-cms-puf-platform-reconciliation T017)."""
...
CSV_CONTENT = (
    ...
    "7777777777,Psych Clinic,Sarah,Psychiatry,MA,90837,Psychotherapy 60 min,"
    "150,600,30000.00,24000.00\n"
)

MODULE = "dk_data.ingestion.sources.cms_mental_health_puf"


def test_loader_returns_success_result_shape():
    from dk_data.ingestion.sources.cms_mental_health_puf import (
        load_cms_mental_health_puf,
    )
    ...
    with patch(f"{MODULE}.get_cursor") as mock_gc, patch(
        ...
        result = load_cms_mental_health_puf(path)
...
def test_loader_does_not_call_log_to_meta():
    ...
    with patch(f"{MODULE}.get_cursor") as mock_gc, patch(
        ...
        mock_gc.return_value = _get_cursor_ctx(0)
        mock_ur.return_value = 1
        load_cms_mental_health_puf(path)
        mock_ltm.assert_not_called()


def test_fetcher_source_name():
    from dk_data.ingestion.fetchers.cms_mental_health_puf import (
        CMSMentalHealthPUFFetcher,
    )

    assert CMSMentalHealthPUFFetcher.SOURCE_NAME == "cms_mental_health_puf"
