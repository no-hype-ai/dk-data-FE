"""Tests for the sec_edgar adapter (#415 WS3 Batch B)."""
...
class TestContract:
    def test_contract(self):
        ...
        a = Adapter()
        assert a.source_name == "sec_edgar"
        assert a.raw_table == "sec_edgar"
        assert a.raw_schema == "mol_raw"
        assert a.normalize({"x": 1}) == {"x": 1}
        assert a.full_table_name == "mol_raw.sec_edgar"
...
class TestDbHit:
    def test_db_hit_returns_dict(self):
        ...
        assert result == {"source": "sec_edgar_local", "results": [{"response_body": {"hit": True}}]}
