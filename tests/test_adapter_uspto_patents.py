"""Tests for the uspto_patents adapter (#415 WS3 Batch C)."""

import asyncio
import pytest

from dk_data.services.mcp.adapters.uspto_patents import Adapter
...
class TestContract:
    def test_contract(self):
        ...
        a = Adapter()
        assert a.source_name == "uspto_patents"
        assert a.raw_table == "uspto_patents"
        assert a.raw_schema == "mol_raw"
        assert a.normalize({"x": 1}) == {"x": 1}
        assert a.full_table_name == "mol_raw.uspto_patents"
...
class TestDbHit:
    def test_db_hit_returns_dict(self):
        ...
        assert result == {"source": "uspto_patents_local", "results": [{"response_body": {"hit": True}}]}
