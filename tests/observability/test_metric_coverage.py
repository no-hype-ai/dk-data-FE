...
DASHBOARD_ALLOWLIST: dict[str, str] = {
    ...
    "DK_HYDRATION_ROW_MISMATCH_TOTAL": "integrity gate (Horizon 1 / plan §B.3) — increments when the post-restore row count deviates from the manifest-declared expected value beyond HYDRATION_ROW_TOLERANCE_PCT. Consumed by the DkHydrationRowMismatch PrometheusRule (defined in PR-304 k8s/apps/observability/alert-rules/) which pages on any non-zero rate. A panel visualizing historical mismatch counts adds no operational value beyond the alert.",
    "DK_HYDRATION_MANIFEST_MISSING_TOTAL": "coverage gauge (Horizon 1 / plan §B.3) — increments when the row-count gate runs against an artifact that has no manifest entry (gate silently proceeds but records the gap). Surfaced via a PrometheusRule that tracks manifest adoption per source; used to drive the migration plan for full manifest coverage, not a live dashboard metric.",
    "DK_WAL_SOURCE_BUDGET_EXHAUSTED_TOTAL": "WAL backpressure anomaly signal (Horizon 2 / plan §C.2) — increments once per (run, source) when a single source exhausts WAL_PAUSE_BUDGET_PER_SOURCE_SECONDS. Consumed by the DkWalSourceBudgetExhausted PrometheusRule (any non-zero rate pages on-call); alert-driven, not a dashboard panel — per-source pause-budget exhaustion is a discrete event, not a trend worth plotting.",
    "DK_HYDRATION_DLQ_ADDED_TOTAL": "DLQ telemetry (Horizon 2 / plan §C.3) — increments on every hydration failure recorded to meta.hydration_backlog. Consumed by the DkHydrationDlqFailureRate PrometheusRule (alerts when rate spikes over baseline), not a dashboard panel — operator response lives in alerting space; the backlog table itself is the visual surface for triage (via the --list-backlog CLI and the dk-data-fe-hydration-backlog dashboard panel that reads meta.hydration_backlog, not this counter).",
    "DK_HYDRATION_DLQ_QUARANTINED_TOTAL": "DLQ quarantine telemetry (Horizon 2 / plan §C.3) — increments once per source quarantine transition. Consumed by the DkHydrationSourceQuarantined PrometheusRule (pages on any non-zero rate — a source entering quarantine is a ticketed event). No dashboard panel: quarantine is a discrete, alert-driven event, not a trend.",
    "DK_HYDRATION_DLQ_SKIPPED_TOTAL": "DLQ skip telemetry (Horizon 2 / plan §C.3) — increments every time the dispatcher skips a quarantined source. Consumed by the DkHydrationDlqSkipRate PrometheusRule (elevated skip rate means a quarantine has been outstanding too long and needs operator attention). No dashboard panel for the same reason as the other DLQ counters — alerting is the UX.",
    "DK_BUDGET_RESERVATION_DENIED_TOTAL": "admission-control anomaly signal (Horizon 3 / plan §D.3) — increments per-budget_key when ResourceBudget.try_reserve() denies capacity. Consumed by the DkBudgetReservationDenied PrometheusRule (a sustained non-zero rate means the dispatcher is being throttled and the budget needs retuning); alert-driven, not a dashboard panel — the dashboard shows DK_BUDGET_REMAINING / DK_BUDGET_TOTAL_CAPACITY which is the admission-decision UX.",
    "DK_HYDRATION_DISPATCH_DEFERRED_TOTAL": "dispatcher-level admission-control anomaly signal (Horizon 3 / plan §D.1 + §D.3) — increments per-source when the hydrate_dispatcher's admission-decorated dispatch callable returns None (ResourceBudget.try_reserve denied capacity). Consumed by the DkHydrationDispatchDeferred PrometheusRule (a sustained non-zero rate for a given source means its consumes: block or meta.resource_budget needs retuning, or the C.5 PgBouncer hydration pool is saturated); alert-driven, not a dashboard panel — per-source deferral is a discrete event, not a trend worth plotting. Complements DK_BUDGET_RESERVATION_DENIED_TOTAL, which is keyed by budget_key instead of source.",
}
