...
DASHBOARD_ALLOWLIST: dict[str, str] = {
    ...
    "DK_BUDGET_REMAINING": "admission-control UX metric (Horizon 3 / plan §D.3) — per-budget_key remaining capacity, set by ResourceBudget.publish_snapshot(). The dk-data-fe-resource-budget dashboard (stub today, content scheduled as a D.3 follow-up) will render this against DK_BUDGET_TOTAL_CAPACITY as a utilization chart. Temporary allowlist until the panel ships.",
    "DK_BUDGET_TOTAL_CAPACITY": "admission-control UX metric (Horizon 3 / plan §D.3) — per-budget_key total capacity, set by ResourceBudget.publish_snapshot(). Denominator for the utilization chart on the dk-data-fe-resource-budget dashboard (stub today, content scheduled as a D.3 follow-up). Temporary allowlist until the panel ships.",
}
