"""Tests for StaffingDecompositionAgent.

Feature: 016-cms-puf-datasource-integration
Task: T093
"""

import os
from unittest.mock import patch

import pytest


class TestStaffingDecompositionAgent:

    @patch.dict(os.environ, {"LITELLM_BASE_URL": "http://test:8000", "LITELLM_API_KEY": "test"})
    @patch("dk_data.agents.base_agent.OpenAI")
    @patch("dk_data.agents.base_agent.psycopg2")
    def test_agent_class_exists(self, mock_pg, mock_openai):
        from dk_data.agents.staffing_decomposition import StaffingDecompositionAgent
        agent = StaffingDecompositionAgent()
        assert agent.AGENT_NAME == "staffing_decomposition"

    @patch.dict(os.environ, {"LITELLM_BASE_URL": "http://test:8000", "LITELLM_API_KEY": "test"})
    @patch("dk_data.agents.base_agent.OpenAI")
    @patch("dk_data.agents.base_agent.psycopg2")
    def test_no_anthropic_import(self, mock_pg, mock_openai):
        import inspect
        from dk_data.agents import staffing_decomposition
        source = inspect.getsource(staffing_decomposition)
        assert "import anthropic" not in source
