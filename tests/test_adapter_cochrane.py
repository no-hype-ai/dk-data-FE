"""Tests for the cochrane adapter (#415 WS3 Batch B)."""

import asyncio
import pytest

from dk_data.services.mcp.adapters.cochrane import Adapter, CochraneTool
...
class TestContract:
    def test_contract(self):
        ...
        a = Adapter()
        assert a.source_name == "cochrane"
        assert a.raw_table == "cochrane_reviews"
        assert a.raw_schema == "mol_raw"
        assert a.normalize({"x": 1}) == {"x": 1}
        assert a.full_table_name == "mol_raw.cochrane_reviews"
...
class TestDbHit:
    def test_db_hit_returns_dict(self):
        ...
        assert result == {"source": "cochrane_local", "results": [{"response_body": {"hit": True}}]}
...
class TestExistingToolIntact:
    def test_cochrane_tool_importable(self):
        assert CochraneTool is not None
