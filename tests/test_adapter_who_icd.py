"""Tests for the who_icd adapter (#415 WS3 Batch D)."""

import asyncio
import pytest

from dk_data.services.mcp.adapters.who_icd import Adapter
...
class TestContract:
    def test_contract(self):
        ...
        a = Adapter()
        assert a.source_name == "who_icd"
        assert a.raw_table == "who_icd"
        assert a.raw_schema == "mol_raw"
        assert a.normalize({"x": 1}) == {"x": 1}
        assert a.full_table_name == "mol_raw.who_icd"
...
class TestDbHit:
    def test_db_hit_returns_dict(self):
        ...
        assert result == {"source": "who_icd_local", "results": [{"response_body": {"hit": True}}]}
