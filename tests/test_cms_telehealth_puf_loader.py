"""Tests for cms_telehealth_puf loader (019-cms-puf-platform-reconciliation T017)."""
...
CSV_CONTENT = (
    ...
    "Place_Of_Srvc,Tot_Benes,Tot_Srvcs,Tot_Mdcr_Alowd_Amt,Tot_Mdcr_Pymt_Amt\n"
    "9999999999,Telehealth MD,Emily,Internal Medicine,CO,99213,"
    "Office Visit,O,100,200,10000.00,8000.00\n"
)

MODULE = "dk_data.ingestion.sources.cms_telehealth_puf"


def test_loader_returns_success_result_shape():
    from dk_data.ingestion.sources.cms_telehealth_puf import load_cms_telehealth_puf
    ...
    with patch(f"{MODULE}.get_cursor") as mock_gc, patch(
        ...
        result = load_cms_telehealth_puf(path)
...
def test_loader_does_not_call_log_to_meta():
    ...
    with patch(f"{MODULE}.get_cursor") as mock_gc, patch(
        ...
        mock_gc.return_value = _get_cursor_ctx(0)
        mock_ur.return_value = 1
        load_cms_telehealth_puf(path)
        mock_ltm.assert_not_called()


def test_fetcher_source_name():
    from dk_data.ingestion.fetchers.cms_telehealth_puf import CMSTelehealthPUFFetcher

    assert CMSTelehealthPUFFetcher.SOURCE_NAME == "cms_telehealth_puf"
