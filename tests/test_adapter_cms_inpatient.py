"""Tests for the cms_inpatient adapter (#415 WS3 Batch E — hcs_raw)."""

import asyncio
import pytest

from dk_data.services.mcp.adapters.cms_inpatient import Adapter
...
class TestContract:
    def test_contract(self):
        ...
        a = Adapter()
        assert a.source_name == "cms_inpatient"
        assert a.raw_table == "cms_medicare_inpatient"
        assert a.raw_schema == "hcs_raw"
        assert a.normalize({"x": 1}) == {"x": 1}
        assert a.full_table_name == "hcs_raw.cms_medicare_inpatient"
...
class TestDbHit:
    def test_db_hit_returns_dict(self):
        ...
        assert result == {"source": "cms_inpatient_local", "results": [{"response_body": {"hit": True}}]}
