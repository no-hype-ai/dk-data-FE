"""Tests for cms_snf_puf loader (019-cms-puf-platform-reconciliation T017)."""
...
def test_loader_returns_success_result_shape():
    from dk_data.ingestion.sources.cms_snf_puf import load_cms_snf_puf
...
def test_loader_does_not_call_log_to_meta():
    ...
    with patch(f"{MODULE}.get_cursor") as mock_gc, patch(
        ...
        mock_gc.return_value = _get_cursor_ctx(0)
        mock_ur.return_value = 1
        load_cms_snf_puf(path)
        mock_ltm.assert_not_called()


def test_fetcher_source_name():
    from dk_data.ingestion.fetchers.cms_snf_puf import CMSSNFPUFFetcher

    assert CMSSNFPUFFetcher.SOURCE_NAME == "cms_snf_puf"
