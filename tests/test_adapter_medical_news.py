"""Tests for the medical_news adapter (#415 WS3 Batch D)."""

import asyncio
import pytest

from dk_data.services.mcp.adapters.medical_news import Adapter
...
class TestContract:
    def test_contract(self):
        ...
        a = Adapter()
        assert a.source_name == "medical_news"
        assert a.raw_table == "medical_news"
        assert a.raw_schema == "mol_raw"
        assert a.normalize({"x": 1}) == {"x": 1}
        assert a.full_table_name == "mol_raw.medical_news"
...
class TestDbHit:
    def test_db_hit_returns_dict(self):
        ...
        assert result == {"source": "medical_news_local", "results": [{"response_body": {"hit": True}}]}
