"""Tests for the euipo_trademarks adapter (#415 WS3 Batch D)."""

import asyncio
import pytest

from dk_data.services.mcp.adapters.euipo_trademarks import Adapter
...
class TestContract:
    def test_contract(self):
        ...
        a = Adapter()
        assert a.source_name == "euipo_trademarks"
        assert a.raw_table == "euipo_trademarks"
        assert a.raw_schema == "mol_raw"
        assert a.normalize({"x": 1}) == {"x": 1}
        assert a.full_table_name == "mol_raw.euipo_trademarks"
...
class TestDbHit:
    def test_db_hit_returns_dict(self):
        ...
        assert result == {"source": "euipo_trademarks_local", "results": [{"response_body": {"hit": True}}]}
