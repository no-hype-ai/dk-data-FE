"""Tests for the drugbank adapter (#415 WS3 Batch A). asyncio.run, fake pool, no respx."""
...
class TestContract:
    def test_contract(self):
        ...
        a = Adapter()
        assert a.source_name == "drugbank"
        assert a.raw_table == "drugbank"
        assert a.raw_schema == "mol_raw"
        assert a.normalize({"x": 1}) == {"x": 1}
        assert a.full_table_name == "mol_raw.drugbank"
...
class TestDbHit:
    def test_db_hit_returns_dict(self):
        ...
        assert result == {"source": "drugbank_local", "results": [{"response_body": {"hit": True}}]}
