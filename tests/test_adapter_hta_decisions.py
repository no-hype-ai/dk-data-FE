"""Tests for the hta_decisions adapter (#415 WS3 Batch B)."""

import asyncio
import pytest

from dk_data.services.mcp.adapters.hta_decisions import Adapter, HtaDecisionsTool
...
class TestContract:
    def test_contract(self):
        ...
        a = Adapter()
        assert a.source_name == "hta_decisions"
        assert a.raw_table == "hta_decisions"
        assert a.raw_schema == "mol_raw"
        assert a.normalize({"x": 1}) == {"x": 1}
        assert a.full_table_name == "mol_raw.hta_decisions"
...
class TestDbHit:
    def test_db_hit_returns_dict(self):
        ...
        assert result == {"source": "hta_decisions_local", "results": [{"response_body": {"hit": True}}]}
...
class TestExistingToolIntact:
    def test_hta_decisions_tool_importable(self):
        assert HtaDecisionsTool is not None
