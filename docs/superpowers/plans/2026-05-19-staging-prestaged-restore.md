...
- **(B) Egress — CORRECTION: no dk-data-FE PR needed.** Verified `git origin/main:k8s/apps/infrastructure/base/networkpolicy.yaml` *already* allows `8333/TCP → infra` **and** `→ infra-staging` (SeaweedFS S3). The LIVE `dk-data-staging/dk-data-egress` lacks it only because it was last applied 2026-03-01 and **staging ArgoCD has not reconciled since** (drift, not a code gap). Earlier "separate egress PR" assessment was wrong — superseded by the root cause below.
- **(D) Ordering — currently safe, same root cause.** SP3 `^fetch-*` suspension is **not live** on staging (0/20 suspended despite `main`==`staging`==`433b70b`); `staging-prestaged-restore` CronJob `NotFound`. Not a timing win — it's the same staging-ArgoCD breakage (below). No active staleness gap, but #435 also won't deploy until it's fixed.

**ROOT CAUSE (verified in argocd ns) — staging ArgoCD is broken (dk-alchemy/GitOps-owned):**
- `dk-data-staging` (project `dk-data`, via app-of-apps `dk-data-bootstrap-staging`): `OutOfSync/Missing`; hard `SyncError` at `433b70b` — **`Job/db-migrate` "reached the specified backoff limit"** blocks the whole staging sync (retried 5×).
- `dk-data-fe-staging` (project `default`, via `dk-root-applicationsets`, annotated `dk-alchemy.io/sync-activation: manual-until-stage-6c`): `OutOfSync/Missing`; 40+ `SharedResourceWarning`s — duplicate Application fighting `dk-data-staging` over the same `k8s/overlays/staging` resources.
- `dk-data-prod` `Synced/Healthy` — only staging. This one fault explains (B), (D), and that **#435 won't deploy to staging post-merge** until resolved. Owner: dk-alchemy/platform (Application/ApplicationSet defs + the `manual-until-stage-6c` migration gate).

**Cross-repo coordination:** mesh tag `dk` (dk-data-FE + dk-alchemy both enrolled). Handoff opened: `~/.dk-mesh/handoffs/dk/open/2026-05-19-dk-data-fe-to-dk-alchemy-staging-postgres-500gi-and-argocd-staging-broken.md` (synced). dk-data-FE status published. dk-alchemy DRAFT PR #800 (storage) carries the node-disk gate.
